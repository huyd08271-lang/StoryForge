...
async function getAIContext(story,chapter){
  if(!story)return {story:null,chapter:null,characters:[],world:[],timeline:[],notes:[],canon:[]};
  const [characters,world,timeline,notes,canon]=await Promise.all([
    supabase.from("characters").select("name,role,age,personality,appearance,relationships,notes").eq("story_id",story.id).limit(30),
    supabase.from("world_entries").select("name,category,description,rules,notes").eq("story_id",story.id).limit(30),
    supabase.from("timeline_events").select("date_label,title,description,characters").eq("story_id",story.id).order("sort_order").limit(30),
    supabase.from("notes").select("title,content,category,status").eq("story_id",story.id).limit(30),
    supabase.from("canon_items").select("key,value,source,locked").eq("story_id",story.id).limit(50)
  ]);
  return {
    story:{id:story.id,title:story.title,genre:story.genre,description:story.description},
    chapter:chapter?{id:chapter.id,number:chapter.number,title:chapter.title,content:chapter.content||""}:null,
    characters:characters.data||[],world:world.data||[],timeline:timeline.data||[],notes:notes.data||[],canon:canon.data||[]
  };
}

async function callStoryForgeAI({action,story,chapter,prompt,selection}){
  const context=await getAIContext(story,chapter);
  const r=await fetch("/api/ai",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({action,prompt,selection,context})});
  const data=await r.json().catch(()=>({}));
  if(!r.ok)throw new Error(data.error||"Không gọi được AI.");
  return data;
}
...
function Editor({story,chapter,setChapter,setChapters}){
  ...
  useEffect(()=>{
    setDraft(chapter?.content||"");setTitle(chapter?.title||"");setWordCount(chapter?.word_count||0);setStatus("Đã lưu");setAiOpen(false);
  },[chapter?.id]);
  ...
  const saveNow=async()=>{
    ...
    const {data,error}=await supabase.from("chapters").update({content:draft,word_count:wc,title:cleanTitle}).eq("id",chapter.id).select().single();
    if(error){console.error(error);setStatus("Lỗi lưu");setSaving(false);return;}
    setWordCount(wc);setTitle(data.title);setChapter(data);setChapters(cs=>cs.map(c=>c.id===data.id?data:c));
    await supabase.from("stories").update({updated_at:new Date().toISOString()}).eq("id",story.id);
    setStatus("Đã lưu cloud");setSaving(false);
  };

  useEffect(()=>{if(!chapter)return;const t=setTimeout(saveNow,800);return()=>clearTimeout(t)},[draft,title,chapter?.id]);

  if(!chapter)return <EmptySelect text="Chọn một chương để bắt đầu viết."/>;
  return <section className="editorPage">
    <div className="editorHead">
      <div><span className="eyebrow">{story?.title}</span><input className="chapterTitleInput" value={title} onChange={e=>setTitle(e.target.value)} onBlur={saveNow}/><div className="editorMeta"><span>{wordCount} từ</span><span>•</span><span className={status.includes("Lỗi")?"bad":"ok"}>● {status}</span></div></div>
      <div className="headerActions"><button className="soft" disabled={saving} onClick={saveNow}>{saving?"Đang lưu…":"Lưu ngay"}</button><button className="ghost" onClick={()=>setChapter(null)}>← Danh sách chương</button></div>
    </div>
    <div className="editorLayout">
      ...
      <aside className="editorSide">
        <div className="sideCard"><h3>✦ Trợ lý AI</h3><p>AI đọc đoạn hiện tại cùng Canon, nhân vật, thế giới và dòng thời gian trước khi đề xuất.</p><button className="soft" onClick={()=>setAiOpen(true)}>Gợi ý cho đoạn này</button></div>
    ...
    {aiOpen&&<AISuggestionModal story={story} chapter={{...chapter,content:draft,title}} onClose={()=>setAiOpen(false)} onInsert={text=>{const join=draft.trim()?`${draft.replace(/\s+$/,"\n\n")} ${text}`:text;setDraft(join);setAiOpen(false);setTimeout(saveNow,0)}}/>}
  </section>
}

function AISuggestionModal({story,chapter,onClose,onInsert}){
  ...
  const run=async()=>{setLoading(true);setError("");try{const r=await callStoryForgeAI({action:mode,story,chapter,prompt:"Hãy viết phần tiếp theo phù hợp với đoạn hiện tại. Không thay đổi Canon. Viết tự nhiên bằng tiếng Việt, ưu tiên văn phong của tác giả.",selection:chapter.content?.slice(-5000)||""});setResult(r.text||"")}catch(e){setError(e.message)}finally{setLoading(false)}};
  useEffect(()=>{run()},[]);
  const labels={continue:"Viết tiếp",suggest:"Gợi ý diễn biến",dialogue:"Gợi ý lời thoại",logic:"Kiểm tra logic"};
  return <Modal title="✦ Trợ lý AI cho đoạn này" onClose={onClose}>
    <p className="muted">AI chỉ đề xuất. Bản thảo chỉ thay đổi khi mày bấm <b>Chèn vào bản thảo</b>.</p>
    <div className="headerActions" style={{marginBottom:12}}>{Object.entries(labels).map(([k,v])=><button key={k} className={mode===k?"":"ghost"} onClick={()=>setMode(k)}>{v}</button>)}</div>
    {loading?<div className="empty" style={{padding:40}}><div className="emptyIcon">✦</div><h3>AI đang đọc ngữ cảnh…</h3><p>Đang kiểm tra Canon, nhân vật, thế giới và chương hiện tại.</p></div>:error?<div className="msg" style={{color:"#b42318"}}>{error}</div>:<textarea rows="15" value={result} onChange={e=>setResult(e.target.value)} placeholder="Kết quả AI…"/>}
    <div className="modalActions"><button className="ghost" onClick={onClose}>Đóng</button><button className="soft" disabled={loading||!result.trim()} onClick={run}>↻ Tạo lại</button><button disabled={loading||!result.trim()} onClick={()=>onInsert(result.trim())}>Chèn vào bản thảo</button></div>
  </Modal>
}
...
function AI({story,chapter}){
  const [prompt,setPrompt]=useState(""),[messages,setMessages]=useState([{role:"assistant",text:"Tao đã sẵn sàng. Hỏi về diễn biến, nhân vật, logic, Canon hoặc yêu cầu viết tiếp."}]),[loading,setLoading]=useState(false);
  const send=async()=>{if(!prompt.trim()||loading)return;const text=prompt.trim();setPrompt("");setMessages(m=>[...m,{role:"user",text}]);setLoading(true);try{const r=await callStoryForgeAI({action:"chat",story,chapter,prompt:text});setMessages(m=>[...m,{role:"assistant",text:r.text||"AI không trả về nội dung."}])}catch(e){setMessages(m=>[...m,{role:"assistant",text:`Lỗi AI: ${e.message}`}])}finally{setLoading(false)}};
  return <section><div className="aiHeader"><div><span className="eyebrow">STORYFORGE AI</span><h2>Trợ lý AI</h2><p className="muted">{story?`Đang làm việc với: ${story.title}`:"Chọn truyện để AI có ngữ cảnh cụ thể."}</p></div></div><div className="aiWorkspace"><div className="chat">{messages.map((m,i)=><div className={`bubble ${m.role}`} key={i}>{m.text}</div>)}{loading&&<div className="bubble assistant">AI đang suy nghĩ…</div>}<div className="promptRow"><textarea value={prompt} onChange={e=>setPrompt(e.target.value)} placeholder="Ví dụ: kiểm tra logic của chương này…" onKeyDown={e=>{if(e.key==="Enter"&&!e.shiftKey){e.preventDefault();send()}}}/><button disabled={loading||!prompt.trim()} onClick={send}>Gửi ↑</button></div></div><div className="aiInfo"><h3>Nguyên tắc AI</h3><ul><li>Không tự ý thay đổi Canon.</li><li>Không tự ý giết/thêm nhân vật lớn.</li><li>Phát hiện mâu thuẫn trước khi đề xuất.</li><li>Ưu tiên gợi ý, không chiếm quyền viết.</li><li>Có thể viết tiếp khi mày yêu cầu.</li></ul></div></div></section>
}
